...
ALTER TABLE portal_users ADD COLUMN IF NOT EXISTS session_token TEXT DEFAULT NULL;
CREATE INDEX IF NOT EXISTS portal_users_reset_token ON portal_users(reset_token) WHERE reset_token IS NOT NULL;
CREATE INDEX IF NOT EXISTS portal_users_session_token ON portal_users(session_token) WHERE session_token IS NOT NULL;
