...
export const handler = async (event) => {
  ...
  // ── SUPABASE PROXY ─────────────────────────────────────────────────────
  if (body._sb) {
    if (!SB_URL) return respond({
      error: "SUPABASE_URL manquant. Ajoutez-le dans Netlify > Site settings > Environment variables, puis redéployez."
    }, 503);
    if (!SB_KEY) return respond({
      error: "SUPABASE_SERVICE_KEY manquant dans les variables Netlify."
    }, 503);
    ...
    // Body uniquement pour les méthodes qui en ont besoin
    ...
    try {
      ...
      // 204 No Content (DELETE/PATCH réussi sans retour)
      if (!text || resp.status === 204) return respond(null, resp.status);

      // Retourner tel quel (Supabase renvoie du JSON)
      return {
